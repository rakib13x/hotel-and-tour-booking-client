...
import VisaBookingQueriesTable from "./VisaBookingQueriesTable";

export default function VisaBookingQueriesClient(): React.ReactElement {
  ...
  const { data: statsData } = useGetVisaBookingQueryStatsQuery();

  const [deleteQuery, { isLoading: isDeleting }] =
    useDeleteVisaBookingQueryMutation();
  const [updateQueryStatus, { isLoading: isUpdating }] =
    useUpdateVisaBookingQueryStatusMutation();
  ...
  const handleStatusUpdate = async (id: string, status: string) => {
    try {
      await updateQueryStatus({ id, data: { status: status as any } }).unwrap();
  ...
  const confirmDelete = async () => {
    ...
    try {
      const result = await deleteQuery(queryToDelete).unwrap();
    ...
    return (
      <div className="py-8 text-center">
        <p className="text-red-500">
          Error loading visa booking queries. Please try again.
        </p>
      </div>
    );
  }
    ...
    <>
      ...
      <div className="mb-6 flex flex-col gap-4 sm:flex-row">
        <div className="relative flex-1">
          ...
          <Input
            placeholder="Search by name, email, phone, country, or visa type..."
            value={searchTerm}
      ...
      <ConfirmationDialog
        ...
        title="Delete Query"
        description="Are you sure you want to delete this visa booking query? This action cannot be undone."
        confirmText="Delete"
        cancelText="Cancel"
